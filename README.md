...
a) by taking the average unemployment per county over 5 years (instead of annually);
b) by focusing on bachelor's attainment rates per county (instead of all levels of education [*e.g.*, less than HS, HS, some college, college and above];
c) and by collapsing eligible voters and registered voters per county into one *registration_rate* variable
